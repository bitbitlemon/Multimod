...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    ctc_a2l_module: object = settings['ctc_a2l_module']
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            if batch_chunk > 1:
                ...
                for i in range(batch_chunk):
                    ...
                    if hyp_params.dataset == 'iemocap':
                        preds_i = preds_i.contiguous().reshape(-1, 2)
                        eval_attr_i = eval_attr_i.contiguous().reshape(-1)
                ...
                if hyp_params.dataset == 'iemocap':
                    preds = preds.contiguous().reshape(-1, 2)
                    eval_attr = eval_attr.contiguous().reshape(-1)
            ...
            if i_batch % hyp_params.log_interval == 0 and i_batch > 0:
                ...
                print('Epoch {:2d} | Batch {:3d}/{:3d} | Time/Batch(ms) {:5.2f} ||00 Train Loss {:5.4f}'.
    ...
    def evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False):
        ...
        # 计算准确率
        if hyp_params.dataset == 'iemocap':
            preds_labels = torch.argmax(results, dim=1)
        else:
            preds_labels = results.round()

        accuracy = accuracy_score(truths.cpu(), preds_labels.cpu())
    ...
    for epoch in range(1, hyp_params.num_epochs + 1):
        start = time.time()
        train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer,
              ctc_criterion)
        val_loss, val_accuracy, _, _ = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=False)
        test_loss, test_accuracy, _, _ = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=True)
        ...
        print("-" * 50)
        print(
            'Epoch {:2d} | Time {:5.4f} sec | Valid Loss {:5.4f} | Valid Accuracy {:5.4f} ||| Test Loss {:5.4f} | Test Accuracy {:5.4f}'.format(
                epoch, duration, val_loss, val_accuracy, test_loss, test_accuracy))
        print("-" * 50)
    ...
    _, test_accuracy, results, truths = evaluate(model, ctc_a2l_module, ctc_v2l_module, criterion, test=True)
